...
void DisplayUI::setup() {
  ...
  createMenu(&mainMenu, NULL, [this]() {
    ...
    /*addMenuNode(&mainMenu, D_CLOCK, [this]() { // CLOCK MONITOR
      mode = DISPLAY_MODE::CLOCK;
      display.setFont(Dialog_plain_6);
      display.setTextAlignment(TEXT_ALIGN_CENTER);
    });*/
    addMenuNode(&mainMenu, S_SYSTEM, &sysMenu); // System information 
  ...
  createMenu(&apListMenu, &showMenu, [this]() {
    ...
    for (int i = 0; i < c; i++) {
      addMenuNode(&apListMenu, [i]() {
        return b2a(accesspoints.getSelected(i)) + accesspoints.getSSID(i) + " [Ch" + accesspoints.getCh(i) + "]";
      }, [this, i]() {
  ...
  // SYS MENU
  createMenu(&sysMenu, &mainMenu, [this]() {
    addMenuNode(&sysMenu, [this]() {
      return leftRight(str(S_MAC_ST), macToStr(settings.getMacSt()), maxLen - 1);
    }, NULL);
    addMenuNode(&sysMenu, [this]() {
      return leftRight(str(S_MAC_AP), macToStr(settings.getMacAP()), maxLen - 1);
    }, NULL);
    addMenuNode(&sysMenu, [this]() {
      return leftRight(str(S_SSID), settings.getSSID(), maxLen - 1);
    }, NULL);
    addMenuNode(&sysMenu, [this]() {
      return str(S_RESET);
    }, [this]() {
...
void DisplayUI::drawMenu() {
  String tmp;
  int    tmpLen;
  int    row = (currentMenu->selected / maxListImtes) * maxListImtes;
  ...
  // draw menu entries
  for (int i = row; i < currentMenu->list->size() && i < row + maxListImtes; i++) {
...
void DisplayUI::drawLoadingScan() {
  ...
  drawString(0, leftRight(str(DSP_SCAN_FOR), scan.getMode() + " " + percentage, maxLen));
